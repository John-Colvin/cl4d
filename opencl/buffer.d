...
struct CLBuffer
{
	...
	CLBuffer createRegionSubBuffer(cl_mem_flags flags, size_t origin, size_t size)
	{
	    ...
	    (
		exceptionHandling(
		    ["CL_INVALID_VALUE",                 "the region specified by (origin, size) is out of bounds in buffer OR buffer "
		                                        ~"was created with CL_MEM_WRITE_ONLY and flags specifies CL_MEM_READ_WRITE or "
		                                        ~"CL_MEM_READ_ONLY, OR if buffer was created with CL_MEM_READ_ONLY and flags "
                                                        ~"specifies CL_MEM_READ_WRITE or CL_MEM_WRITE_ONLY, OR if flags specifies "
		    ...
		    ["CL_MISALIGNED_SUB_BUFFER_OFFSET",  "there are no devices in context associated with buffer for which the origin "
...
struct CLBufferGL
{
	...
	(
		...
		["CL_INVALID_GL_OBJECT",  "bufobj is not a GL buffer object or is a GL buffer object but does not have "
