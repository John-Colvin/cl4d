...
class CLUnrecognizedException : CLException
{
    this(cl_errcode errcode, string file = "", size_t line = 0)
    {
	super(errcode, "unrecognized OpenCL exception occured: " ~ to!string(errcode), file, line);
    }
}

/// a version OpenCL exception
class CLVersionException : CLException
{
    this(string file = "", size_t line = 0)
    {
	super(errcode, "Function is not available for loaded CLVersion " ~ to!string(DerelictCL.loadedVersion), file, line);
    }
}

/// platform exceptions base class
class CLPlatformException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
    }
}

/// device exceptions base class
class CLDeviceException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLContextException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLEventException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLProgramException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLBufferException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLKernelException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
class CLCommandQueueException : CLException
{
    this(cl_errcode errcode, string msg = "", string file = "", size_t line = 0)
    {
	super(errcode, msg, file, line);
...
private string mixinExceptionClasses(E...)(E es)
{
    ...
    foreach(e; es)
    {
        ...
        res ~= `final class CL` ~ toCamelCase(e.name[2..$]) ~ `Exception : ` ~ e.baseclass ~
               `{
                    this(string msg = "", string file = "", size_t line = 0)
                    {
                        super(` ~ e.name ~ `, ` ~ ((e.msg != "") ? `"` ~ e.msg ~ `" ~ ` : "") ~ `msg, file, line);
                    }
                }`;
...
mixin(mixinExceptionClasses(
	  ...
	  ECD("CL_INVALID_IMAGE_DESCRIPTOR", "", "CLBufferException")
	  ));
